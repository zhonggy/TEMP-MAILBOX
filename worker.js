...
export default {
  ...
  async fetch(request, env) {
    ...
    const authed = String(env.ACCESS_PASSWORD || "")
      ? this.getCookie(request, ACCESS_COOKIE) === "1"
      : false;
    ...
    const stats = await this.getMailStats(env);
    const pageAuthed = box ? true : authed;
      ...
      for (const row of results || []) {
        if (row.r2Key) {
          try {
            ...
            if (object) {
              const stored = await object.json();
              for (const file of stored.attachments || []) {
                if (file.key) await env.MAIL_BUCKET.delete(file.key);
              }
            }
          } catch {}
  ...
  getHTML({ box = "", host = "", missingDb = false, stats = EMPTY_STATS, authed = true, passwordError = false }) {
...
<head>
  ...
  <style>
    *{box-sizing:border-box}
    ...
    .body{display:none;margin-top:14px;padding-top:14px;border-top:1px solid #e5e7eb;line-height:1.7;white-space:pre-wrap}
    ...
    @media(max-width:640px){.title{font-size:24px}.email{font-size:19px}}
  </style>
</head>
<body>
  ...
  <div class="wrap">
    ...
    <div class="card">
      ...
      ${!missingDb && authed && box ? `
        ...
        <div class="hint">
          <span id="refreshCountdown">10</span> 秒自动刷新收件
        </div>
